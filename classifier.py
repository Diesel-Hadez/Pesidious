...
def parse_args():
    ...
    parser.add_argument("--logfile", help="The file path to store the logs. (default : classifier_logs_" +
                        str(date.today()) + ".log)", type=Path, default=Path("classifier_logs_" + str(date.today()) + ".log"))
...
def get_label_local(bytez, local_model_threshold):
    # mimic black box by thresholding here
    score = get_score_local(bytez)
    label = float(score >= local_model_threshold)
...
def main():
    ...
    if (args.f):
        info(
            f"[*] Reading File : [bold green]{str(args.f)}", extra={"markup": True})
        with open(args.f, 'rb') as infile:
            bytez = infile.read()

        if (args.remote_model_config):
            info("[*] Querying Remote Model ...")
            private_data = json.load(
                open(os.path.join(module_path, 'params.json'), 'r'))
    ...
    info("[bold green][+] Binary classifer completed succesfully ...",
         extra={"markup": True})
